...
public class FadHelper {
	...
	/**
	 * 将AbstractBasePOJO的数组,根据ID转化为HashMap
	 * 
	 * @param al
	 * @return
	 */
	public static <T> HashMap<String, T> buildHashMapIdToPOJO(ArrayList<T> al) {
	...
	public static String splitShowValue(String showValue, int length) {
		if (length < 0) {
			length = FadHelper.splitLength;
		}
		String returnString = showValue;
		if (StringUtils.isBlank(showValue)) {
			returnString = "";
		} else {
			returnString = FadHelper.substring(showValue, length);
	...
	*/
	public static String substring(String origin, int len) {
		...
		try {
			if (len > FadHelper.length(origin)) {
				return origin;
			}
			len = FadHelper.getSplitLength(origin, len);
